...
class StampaAlbero {
    ...
    public static <T> void stampaNodo(AlberoDiRicerca radice) {
        int livello = StampaAlbero.livelloMassimo(radice);
        stampaNodoRic(Collections.singletonList(radice), 1, livello);
    }

    private static <T> void stampaNodoRic(List<AlberoDiRicerca> nodi, int livello, int livelloMassimo) {
        ...
        List<AlberoDiRicerca> nuoviNodi = new ArrayList<>();

        for (AlberoDiRicerca nodoCorrente : nodi) {
            if (nodoCorrente != null) {
                System.out.print(nodoCorrente.informazione);
    ...
    private static <T> int livelloMassimo(AlberoDiRicerca node) {
        if (node == null) {
            return 0;
        }
